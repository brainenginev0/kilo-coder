...
export function ChatPanel({ messages, activity, isGenerating, providerReady, onSubmit }: ChatPanelProps) {
  ...
  return (
    <section className="flex min-h-0 min-w-0 flex-1 flex-col border-r border-white/[0.07] bg-[#0a0f1d] lg:w-[360px] lg:flex-none xl:w-[390px]">
      <div className="flex h-11 shrink-0 items-center justify-between border-b border-white/[0.07] px-4">
        ...
        <span className={`flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-[0.13em] ${providerReady ? "text-emerald-300" : "text-amber-300"}`}><span className={`size-1.5 rounded-full ${providerReady ? "bg-emerald-300 shadow-[0_0_8px_#6ee7b7]" : "bg-amber-300 shadow-[0_0_8px_#fcd34d]"}`} />{providerReady ? "Live model" : "Demo mode"}</span>
      </div>
      ...
      <div className="shrink-0 border-t border-white/[0.07] bg-[#0b101f] p-3">
        <div className="mb-2 flex items-center gap-1.5 overflow-x-auto pb-1 [scrollbar-width:none]">
          <Suggestion label="Add auth" onClick={() => onSubmit("Add passwordless authentication with protected routes.")} />
          <Suggestion label="Connect a DB" onClick={() => onSubmit("Connect a typed Postgres database and add seed data.")} />
          <Suggestion label="Write tests" onClick={() => onSubmit("Add a focused test suite for the launchpad metrics.")} />
        </div>
        <form onSubmit={submit} className="rounded-xl border border-white/10 bg-[#11182a] p-2 shadow-[0_10px_35px_rgba(0,0,0,0.18)] focus-within:border-violet-300/45">
          <Textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Describe a change to your app..." className="min-h-[62px] resize-none border-0 bg-transparent px-1 py-1.5 text-xs leading-relaxed text-slate-200 shadow-none placeholder:text-slate-600 focus-visible:ring-0" disabled={isGenerating} />
          <div className="mt-2 flex items-center justify-between"><div className="flex items-center gap-1"><button type="button" className="grid size-7 place-items-center rounded-md text-slate-600 transition-colors hover:bg-white/[0.06] hover:text-slate-300" aria-label="Attach file"><Paperclip className="size-3.5" /></button><span className="font-mono text-[9px] text-slate-700">Enter to send</span></div><button type="submit" disabled={!prompt.trim() || isGenerating} className="grid size-7 place-items-center rounded-md bg-cyan-300 text-[#07101a] transition-all hover:bg-cyan-200 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500"><ArrowUp className="size-4" /></button></div>
        </form>
...
function Suggestion({ label, onClick }: { label: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="flex shrink-0 items-center gap-1.5 rounded-md border border-white/[0.07] bg-white/[0.025] px-2 py-1.5 text-[10px] text-slate-500 transition-colors hover:border-violet-300/25 hover:bg-violet-300/10 hover:text-violet-200"><Sparkles className="size-3" />{label}</button>;
}

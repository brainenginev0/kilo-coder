...
export function TopBar({ viewport, onViewportChange, onShare, onDeploy }: TopBarProps) {
  return (
    <header className="flex min-h-[64px] shrink-0 items-center justify-between gap-3 border-b border-white/[0.07] bg-[#0c1121]/90 px-4 backdrop-blur-xl sm:px-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <button className="grid size-8 place-items-center rounded-md text-slate-500 hover:bg-white/[0.05] hover:text-slate-200" aria-label="More actions"><MoreHorizontal className="size-4" /></button>
      </div>
    </header>
  );
}
